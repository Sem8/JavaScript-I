...
console.log(`Make of last car is ${lastCar.car_make} and model is ${lastCar.car_model}`);
...
let carModels = inventory.sort(function(a, b) {
  ...
  for (let i = 0; i < carModels.length; i++) {
      let eachSortedCar = carModels[i];
      console.log(eachSortedCar.car_model);
      }
  
// console.log(eachSortedCar.car_model);
// let carModels = inventory.sort(this.car_model);
...
for (let j = 0; j < inventory.length; j++) {
    let eachCarYear = inventory[j].car_year;
    carYears.push(eachCarYear);    
}
...
let stringifiedBMWAndAudi = JSON.stringify(BMWAndAudi)
console.log(stringifiedBMWAndAudi);
